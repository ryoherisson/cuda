...
__global__ void reduction_kernel_complete_template(int *input, int *temp,
                                                   int size) {
  int tid = threadIdx.x;
...
int main(int argc, char **argv) {
  ...
  switch (block_size) {
  case 1024:
    reduction_kernel_complete_template<1024>
        <<<grid, block>>>(d_input, d_temp, size);
    break;
  case 512:
    reduction_kernel_complete_template<512>
        <<<grid, block>>>(d_input, d_temp, size);
    break;
  case 256:
    reduction_kernel_complete_template<256>
        <<<grid, block>>>(d_input, d_temp, size);
    break;
  case 128:
    reduction_kernel_complete_template<128>
        <<<grid, block>>>(d_input, d_temp, size);
    break;
  case 64:
    reduction_kernel_complete_template<64>
        <<<grid, block>>>(d_input, d_temp, size);
    break;
  }
