...
__global__ void reduction_neighbored_pairs_improved_1(int *int_array,
                                                      int *temp_array,
                                                      int size) {
  int tid = threadIdx.x;
  int gid = blockDim.x * blockIdx.x + threadIdx.x;
...
__global__ void reduction_interleaved_pairs_1(int *int_array, int *temp_array,
                                              int size) {
  int tid = threadIdx.x;
  int gid = blockDim.x * blockIdx.x + threadIdx.x;
...
__global__ void reduction_interleaved_unrolling_blocks2_1(int *input, int *temp,
                                                          int size) {
  int tid = threadIdx.x;
...
__global__ void reduction_interleaved_unrolling_blocks8_1(int *input, int *temp,
                                                          int size) {
  int tid = threadIdx.x;
...
__global__ void reduction_kernel_complete_unrolling8_1(int *input, int *temp,
                                                       int size) {
  int tid = threadIdx.x;
  int index = blockDim.x * blockIdx.x * 8 + threadIdx.x;
...
__global__ void reduction_kernel_complete_template8_1(int *input, int *temp,
                                                      int size) {
  int tid = threadIdx.x;
  int index = blockDim.x * blockIdx.x * 8 + threadIdx.x;
...
int main(int argc, char **argv) {
  ...
  reduction_interleaved_unrolling_blocks2_1<<<new_grid2, block>>>(d_input,
                                                                  d_temp, size);
  gpuErrchk(cudaDeviceSynchronize());
  gpuErrchk(
      cudaMemcpy(h_ref, d_temp, temp_array_byte_size, cudaMemcpyDeviceToHost));
  ...
  reduction_interleaved_unrolling_blocks8_1<<<new_grid8, block>>>(d_input,
                                                                  d_temp, size);
  gpuErrchk(cudaDeviceSynchronize());
  gpuErrchk(
      cudaMemcpy(h_ref, d_temp, temp_array_byte_size, cudaMemcpyDeviceToHost));
  ...
  reduction_kernel_interleaved_warp_unrolling8_1<<<new_grid8, block>>>(
      d_input, d_temp, size);
  gpuErrchk(cudaDeviceSynchronize());
  gpuErrchk(
      cudaMemcpy(h_ref, d_temp, temp_array_byte_size, cudaMemcpyDeviceToHost));
  ...
  reduction_kernel_complete_unrolling8_1<<<new_grid8, block>>>(d_input, d_temp,
                                                               size);
  gpuErrchk(cudaDeviceSynchronize());
  gpuErrchk(
      cudaMemcpy(h_ref, d_temp, temp_array_byte_size, cudaMemcpyDeviceToHost));
  ...
  switch (block_size) {
  case 1024:
    reduction_kernel_complete_template8_1<1024>
        <<<new_grid8, block>>>(d_input, d_temp, size);
    break;
  case 512:
    reduction_kernel_complete_template8_1<512>
        <<<new_grid8, block>>>(d_input, d_temp, size);
    break;
  case 256:
    reduction_kernel_complete_template8_1<256>
        <<<new_grid8, block>>>(d_input, d_temp, size);
    break;
  case 128:
    reduction_kernel_complete_template8_1<128>
        <<<new_grid8, block>>>(d_input, d_temp, size);
    break;
  }
  gpuErrchk(cudaDeviceSynchronize());
